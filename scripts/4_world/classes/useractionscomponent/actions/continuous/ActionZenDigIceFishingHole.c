class ActionZenDigIceFishingHoleCB : ActionContinuousBaseCB
{
	...
	{
		float time = 30; // 30 secs for anything other than iceaxe, iceaxe = 50% duration
...
class ActionZenDigIceFishingHole : ActionContinuousBase
{
	...
	override void Start(ActionData action_data)
	{
		...
		{
			ZenFunctionsIceLakes.OrientObjectToPosition(action_data.m_Player, action_data.m_Target.GetObject().GetPosition());
		}
	}
	...
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		newIce.SetPosition(targetObject.GetPosition() - "0 0.001 0"); // Not sure why... objects are identical .p3d's except for hole cut in one, but they don't align quite properly
		newIce.Update();
		...
		float dmg = -0.4; // 40% damage for pickaxe/any other item
